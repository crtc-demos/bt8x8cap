...
#define SERVICE_KEY L"SYSTEM\\CurrentControlSet\\Services\\"
...
static BOOL CheckDriverImagePath( void )
{
   ...
   if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, SERVICE_KEY NT_DRIVER_NAME, 0, KEY_READ, &hKey) == ERROR_SUCCESS)
   {
      ...
      if (RegQueryValueEx(hKey, L"ImagePath", 0, &dwType, reinterpret_cast<BYTE*>(name_buf), &dwSize) == ERROR_SUCCESS)
      {
         if (dwType == REG_EXPAND_SZ)
         {
            name_buf[dwSize] = 0;

            LOG(1,L"CheckDriverImagePath: registered " NT_DRIVER_NAME L" ImagePath='%s'", name_buf);
            ...
            LOG(1,L"CheckDriverImagePath: service registry key has unexpected type %d", dwType);
      }
      else
         LOG(1,L"CheckDriverImagePath: ImagePath key value not found", name_buf);

      RegCloseKey(hKey);
   }
   else
      LOG(1,L"CheckDriverImagePath: key " SERVICE_KEY NT_DRIVER_NAME L" not found", name_buf);

   return result;
}


const WCHAR* DsDrvGetErrorMsg( DWORD loadError )
{
   const WCHAR* errmsg;
   ...
   switch (loadError)
   {
      case HWDRV_LOAD_NOPERM:
         errmsg = L"Failed to load the TV card driver: access denied.\n"
                  L"On WinNT and Win2K you need admin permissions\n"
                  L"to start the driver.  See README.txt for more info.";
         break;
      case HWDRV_LOAD_MISSING:
         errmsg = L"Failed to load the TV card driver: could not start the service.\n"
                  L"The the driver files dsdrv4.sys and dsdrv4.vxd may not be\n"
                  L"in the nxtvepg directory. See README.txt for more info.";
         break;
      case HWDRV_LOAD_START:
         errmsg = L"Failed to load the TV card driver: could not start the service.\n"
                  L"See README.txt for more info.";
         break;
      case HWDRV_LOAD_INSTALL:
         errmsg = L"Failed to load the TV card driver: could not install\n"
                  L"the service.  See README.txt for more info.";
         break;
      case HWDRV_LOAD_CREATE:
         errmsg = L"Failed to load the TV card driver: Another application may\n"
                  L"already be using the driver. See README.txt for more info.\n";
         break;
      case HWDRV_LOAD_REMOTE_DRIVE:
         errmsg = L"Failed to load the TV card driver: Cannot install the driver\n"
                  L"on a network drive. See README.txt for more info.\n";
         break;
      case HWDRV_LOAD_VERSION:
         errmsg = L"Failed to load the TV card driver: it's is an incompatible\n"
                  L"version. See README.txt for more info.\n";
         break;
      case HWDRV_LOAD_OTHER:
      default:
         errmsg = L"Failed to load the TV card driver.\n"
                  L"See README.txt for more info.";
         break;
   }
   return errmsg;
...
DWORD DsDrvLoad( void )
{
    ...
    if (result != HWDRV_LOAD_SUCCESS)
    {
             ...
             (result == HWDRV_LOAD_VERSION) )
        {
            ...
            LOG(1,L"Attempting Bt8x8 driver recovery: uninstalling & installing driver");
...
static void DsDrv_AddElement( CARD_STATE_BUF * pStateBuf, DWORD value )
{
   ...
   if (pStateBuf->fillCount > pStateBuf->maxCount)
      LOG(1,L"DsDrv-AddElement: illegal fill count %d >= %d", pStateBuf->fillCount, pStateBuf->maxCount);

   if (pStateBuf->fillCount == pStateBuf->maxCount)
   {
      pPrevValues = pStateBuf->pValues;
      pStateBuf->maxCount += STATE_CLUSTER_SIZE;
      pStateBuf->pValues = static_cast<DWORD*>(malloc(pStateBuf->maxCount * sizeof(*pStateBuf->pValues)));
...
void ManageDword( DWORD Offset )
{
   if (cardStateReading)
   {
      if (cardStateReadIdx < cardStateBuf.fillCount)
      {
         ...
         LOG(1, L"Manage-Dword: illegal read index %d >= %d", cardStateReadIdx, cardStateBuf.fillCount);
   }
   else
   {
...
void ManageWord( DWORD Offset )
{
   if (cardStateReading)
   {
      if (cardStateReadIdx < cardStateBuf.fillCount)
      {
         ...
         LOG(1, L"Manage-Word: illegal read index %d >= %d", cardStateReadIdx, cardStateBuf.fillCount);
   }
   else
   {
...
void ManageByte( DWORD Offset )
{
   if (cardStateReading)
   {
      if (cardStateReadIdx < cardStateBuf.fillCount)
      {
         ...
         LOG(1, L"Manage-Byte: illegal read index %d >= %d", cardStateReadIdx, cardStateBuf.fillCount);
   }
   else
   {
