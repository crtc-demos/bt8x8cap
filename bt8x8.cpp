...
//
#define VBI_LINES_PER_FIELD      480 /*16*/
...
static BOOL Bt8x8_AllocDmaMemory( void )
{
   ...
   {
      MessageBox(NULL, L"Failed to allocate RISC code memory: driver abort", L"nxtvepg driver problem", MB_ICONSTOP | MB_OK | MB_TASKMODAL | MB_SETFOREGROUND);
      Bt8x8_FreeDmaMemory();
      return FALSE;
   }
   ...
   for (idx=0; idx < VBI_FRAME_CAPTURE_COUNT; idx++)
   {
      ...
      {
         MessageBox(NULL, L"VBI Memory for DMA not Allocated", L"nxtvepg driver problem", MB_ICONSTOP | MB_OK | MB_TASKMODAL | MB_SETFOREGROUND);
         Bt8x8_FreeDmaMemory();
         return FALSE;
      }
   }
...
static bool CreateRiscCode( void )
{
   ...
   pRiscCode         = static_cast<DWORD*>(HwMem_GetUserPointer(&m_RiscDMAMem));
   ...
   for (nField = 0; nField < VBI_FIELD_CAPTURE_COUNT; nField++)
   {
      ...
      *(pRiscCode++) = 0;
      ...
      pVbiUser = static_cast<BYTE*>(HwMem_GetUserPointer(&m_VBIDMAMem[nField / 2]));
...
static BOOL Bt8x8_SetCapture(BOOL enable)
{
   ...
   if (enable)
   {
      ...
      if (CreateRiscCode())
      {
         ...
         MaskDataByte(BT848_CAP_CTL, BT848_CAP_CTL_CAPTURE_EVEN | BT848_CAP_CTL_CAPTURE_ODD /*| BT848_CAP_CTL_CAPTURE_VBI_EVEN | BT848_CAP_CTL_CAPTURE_VBI_ODD*/, 0x0f);
...
static bool Bt8x8_ResetHardware( void )
{
   ...
   WriteByte (BT848_E_VDELAY_LO, VDELAY & 0xff /*0x20*/);
   WriteByte (BT848_O_VDELAY_LO, VDELAY & 0xff /*0x20*/);
      ...
      if (pVbiBuf != NULL)
      {
         ...
         else if (CurFrame != OldFrame)
         {
            do
            {
               ...
               pVBI = static_cast<BYTE*>(HwMem_GetUserPointer(&m_VBIDMAMem[OldFrame]));
                ...
                {
                    if (capture)
                        fwrite(pVBI, VBI_LINE_SIZE, 1, outputFile);
                }
...
static bool Bt8x8_Open( TVCARD * pTvCard, bool wdmStop )
{
   ...
   if ((DmaCtl & 3) && (CapCtl & 0x0f))
   {
      ...
      MessageBox(NULL, L"Capturing is already enabled in the TV card!\n"
                       L"Probably another video application is running,\n"
                       L"however nxtvepg requires exclusive access.\n"
                       L"Aborting data acquisition.",
                       L"nxtvepg driver problem", MB_ICONSTOP | MB_OK | MB_TASKMODAL | MB_SETFOREGROUND);
      CardConflictDetected = TRUE;
      result = FALSE;
   }
   else
