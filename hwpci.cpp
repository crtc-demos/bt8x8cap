                       ...
                       BOOL supportsAcpi, HWPCI_RESET_CHIP_CB pResetCb)
{
    ...
    if ( dwStatus == ERROR_SUCCESS)
    {
        ...
        {
            LOG(1, L"MapMemory failed 0x%x", dwStatus);
        }
    }
    else
    {
        LOG(1, L"GetPCIInfo failed for %X %X failed 0x%x", VendorID, DeviceID, dwStatus);
    }
    return m_bOpen;
}
...
void HwPci_ClosePCICard( void )
{
    ...
    if (m_MemoryBase != 0)
    {
        ...
        if (dwStatus != ERROR_SUCCESS)
        {
            LOG(1, L"UnmapMemory failed 0x%x", dwStatus);
        }
...
void HwPci_WriteByte(DWORD Offset, BYTE Data)
{
    ...
    if (dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"WriteMemoryBYTE failed 0x%x", dwStatus);
    }
}
...
void HwPci_WriteWord(DWORD Offset, WORD Data)
{
    ...
    if (dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"WriteMemoryWORD failed 0x%x", dwStatus);
    }
}
...
void HwPci_WriteDword(DWORD Offset, DWORD Data)
{
    ...
    if (dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"WriteMemoryDWORD failed 0x%x", dwStatus);
    }
}
...
BYTE HwPci_ReadByte(DWORD Offset)
{
    ...
    if (dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"ReadMemoryBYTE failed 0x%x", dwStatus);
    }
    return bValue;
}
...
WORD HwPci_ReadWord(DWORD Offset)
{
    ...
    if (dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"ReadMemoryWORD failed 0x%x", dwStatus);
    }
    return wValue;
}
...
DWORD HwPci_ReadDword(DWORD Offset)
{
    ...
    if (dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"ReadMemoryDWORD failed 0x%x", dwStatus);
    }
    return dwValue;
}
...
BOOL HwPci_GetPCIConfig(PCI_COMMON_CONFIG* pPCI_COMMON_CONFIG, DWORD Bus, DWORD Slot)
{
    ...
    {
        LOG(1, L"HwPci-GetPCIConfig failed. pPCI_COMMON_CONFIG == NULL");
        return FALSE;
    }
    ...
    if(dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"HwPci-GetPCIConfig failed for %X %X failed 0x%x", Bus, Slot, dwStatus);
        return FALSE;
    }
    return TRUE;
}
...
BOOL HwPci_SetPCIConfig(PCI_COMMON_CONFIG* pPCI_COMMON_CONFIG, DWORD Bus, DWORD Slot)
{
    ...
    if(pPCI_COMMON_CONFIG == NULL)
    {
        LOG(1, L"HwPci-SetPCIConfig failed. pPCI_COMMON_CONFIG == NULL");
        return FALSE;
    }
    ...
    if(dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"HwPci-SetPCIConfig failed for %X %X failed 0x%x", Bus, Slot, dwStatus);
        return FALSE;
    }
    return TRUE;
}
...
BOOL HwPci_GetPCIConfigOffset(BYTE* pbPCIConfig, DWORD Offset, DWORD Bus, DWORD Slot)
{
    ...
    if(pbPCIConfig == NULL)
    {
        LOG(1, L"GetPCIConfigOffset failed. pbPCIConfig == NULL");
        return FALSE;
    }
    ...
    if(dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"GetPCIConfigOffet failed for %X %X failed 0x%x", Bus, Slot, dwStatus);
        return FALSE;
    }
    return TRUE;
}
...
BOOL HwPci_SetPCIConfigOffset(BYTE* pbPCIConfig, DWORD Offset, DWORD Bus, DWORD Slot)
{
    ...
    if(pbPCIConfig == NULL)
    {
        LOG(1, L"SetPCIConfigOffset failed. pbPCIConfig == NULL");
        return FALSE;
    }
    ...
    if(dwStatus != ERROR_SUCCESS)
    {
        LOG(1, L"SetPCIConfigOffset failed for %X %X failed 0x%x", Bus, Slot, dwStatus);
        return FALSE;
    }
    return TRUE;
}
...
static int HwPci_GetACPIStatus( void )
{
    ...
    if(HwPci_GetPCIConfigOffset(&ACPIStatus, 0x50, m_BusNumber, m_SlotNumber))
    {
        ...
        LOG(1, L"Bus %d Card %d ACPI status: D%d", m_BusNumber, m_SlotNumber, ACPIStatus);
        return ACPIStatus;
    }
...
static void HwPci_SetACPIStatus(int ACPIStatus)
{
    ...
    if(HwPci_GetPCIConfigOffset(&ACPIStatusNew, 0x50, m_BusNumber, m_SlotNumber))
    {
        ...
        LOG(1, L"Attempting to set Bus %d Card %d ACPI status to D%d", m_BusNumber, m_SlotNumber, ACPIStatusNew);
        ...
        LOG(1, L"Set ACPI status complete");
    }
}
