...
static WCHAR       m_NTDriverName[MAX_PATH];
...
void HwDrv_Create( void )  //CHardwareDriver::CHardwareDriver()
{
    ...
    wcscpy(m_NTDriverName, L"DSDrv4");
    ...
    //if (arch64Bit)
    //{
    //    wcscat(m_NTDriverName, arch64Bit);
    //}
}
...
void HwDrv_Destroy( void )  //CHardwareDriver::~CHardwareDriver()
{
    ...
    LOG(1,L"(NT driver) ALWAYS_UNINSTALL_NTDRIVER");
...
DWORD HwDrv_LoadDriver( void )
{
    ...
    if (!m_bWindows95)
    {
        LOG(2,L"LoadDriver: Opening the Service Control Manager...");
        ...
        if(hSCManager == NULL)
        {
            LOG(1, L"OpenSCManager returned an error = 0x%X", GetLastError());
            bError = TRUE;
        }

        if(!bError)
        {
            LOG(2,L"LoadDriver: Open the Service " NT_DRIVER_NAME L"...");
            ...
            if(m_hService == NULL)
            {
                ...
                {
                    LOG(1,L"(NT driver) Service does not exist: trying to install it...");

                    loadError = HwDrv_InstallNTDriver();
                    if (loadError != HWDRV_LOAD_SUCCESS)
                    {
                        LOG(1,L"Failed to install NT driver. Giving up.");
                        bError = TRUE;
                    }
                }
                else if(Err == ERROR_ACCESS_DENIED)
                {
                    LOG(1,L"(NT driver) Unable to open service: access denied");
                    loadError = HWDRV_LOAD_NOPERM;
                    bError = TRUE;
                }
                else
                {
                    LOG(1,L"(NT driver) Unable to open service. 0x%X",Err);
                    bError = TRUE;
                }
            }
        }
        ...
        if(hSCManager != NULL)
        {
            ...
            {
                LOG(1,L"(NT driver) Failed to close handle to service control manager.");
                bError = TRUE;
            }
            hSCManager = NULL;
        }
        ...
        if(!bError)
        {
            LOG(2,L"LoadDriver: Starting the Service...");
            if(StartService(m_hService, 0, NULL) == FALSE)
            {
                DWORD Err = GetLastError();
                if ((Err == ERROR_PATH_NOT_FOUND) || (Err == ERROR_FILE_NOT_FOUND))
                {
                    LOG(1, L"StartService() failed: file not found");
                    loadError = HWDRV_LOAD_MISSING;
                    bError = TRUE;
                }
                else if (Err != ERROR_SERVICE_ALREADY_RUNNING)
                {
                    LOG(1, L"StartService() failed 0x%X", Err);
                    loadError = HWDRV_LOAD_START;
                    bError = TRUE;
                }
                else
                {
                    LOG(1, L"Service already started");
                }
            }
            if(!bError)
            {
                LOG(2,L"LoadDriver: Opening the DSDrv4.sys driver file...");
                m_hFile = CreateFile(
                                     L"\\\\.\\DSDrv4",
    ...
    else
    {
        ...
        LOG(2,L"LoadDriver: Opening the DSDrv4.vxd driver file...");
        m_hFile = CreateFile(
                                L"\\\\.\\DSDrv4.VXD",
                                0,
                                0,
    ...
    if(!bError)
    {
        if(m_hFile != INVALID_HANDLE_VALUE)
        {
            ...
            LOG(2,L"LoadDriver: Checking the driver version...");
            HwDrv_SendCommandEx(
            ...
            {
                LOG(1, L"We've loaded up an incompatible version of the driver");
            ...
            {
                LOG(2, L"Found driver version 0x%X", dwVersion);
            }
        }
        else
        {
            LOG(1, L"CreateFile on Driver failed 0x%X", GetLastError());
    ...
    if(bError)
    {
        LOG(2,L"LoadDriver: Error - unloading the driver...");
        HwDrv_UnloadDriver();
    ...
    {
        LOG(1, L"Hardware driver loaded successfully.");
...
void HwDrv_UnloadDriver( void )
{
    ...
    if (!m_bWindows95)
    {
        if (m_hService != NULL)
        {
            ...
            LOG(2,L"UnloadDriver: stopping the service...");
            if(ControlService(m_hService, SERVICE_CONTROL_STOP, &ServiceStatus ) == FALSE)
            {
                LOG(1,L"SERVICE_CONTROL_STOP failed, error 0x%X", GetLastError());
            }
            #endif
            ...
            if(m_hService != NULL)
            {
                ...
                {
                    LOG(1,L"CloseServiceHandle failed, error 0x%X", GetLastError());
                }
                m_hService = NULL;
            }
        }
...
DWORD HwDrv_InstallNTDriver( void )
{
    ...
    LOG(1, L"Attempting to install NT driver.");

    HwDrv_UnloadDriver();

    if (m_bWindows95)
    {
        LOG(1, L"No need to install NT driver with win9x/ME.");
        return TRUE;
    }

    if (!GetModuleFileName(NULL, szDriverPath, sizeof(szDriverPath)))
    {
        LOG(1, L"cannot get module file name. 0x%X",GetLastError());
        szDriverPath[0] = '\0';
        bError = TRUE;
    }
    ...
    if(!bError)
    {
        pszName = szDriverPath + wcslen(szDriverPath);
        ...
        {
            LOG(1, L"InstallNTDriver: cannot install on remote drive: %s", szDriverPath);
    ...
    if(!bError)
    {
        wcscat(szDriverPath, m_NTDriverName);
        wcscat(szDriverPath, L".sys");       
        
        LOG(2,L"InstallNTDriver: Opening the service control manager...");
    ...
    if(!bError)
    {
        ...
        LOG(2,L"InstallNTDriver: Creating the service %s (kernel driver, manual start)",szDriverPath);
        ...
        if(m_hService == NULL)
        {
            ...
            {              
                LOG(2,L"InstallNTDriver: Create failed: already exists: will attempt to delete the service");

                LOG(2,L"InstallNTDriver: Opening the service " NT_DRIVER_NAME L"...");
                m_hService = OpenService(hSCManager, m_NTDriverName, SERVICE_ALL_ACCESS);

                LOG(2,L"InstallNTDriver: Deleting the service...");
                if(DeleteService(m_hService) == FALSE)
                {
                    LOG(1,L"InstallNTDriver: DeleteService failed, error 0x%X", GetLastError());
                    bError = TRUE;
                }
                else
                    LOG(2,L"InstallNTDriver: Succeeded to delete the service.");
                ...
                if(!bError)
                {
                    LOG(2,L"InstallNTDriver: 2nd attempt at creating service %s (kernel driver, manual start)",szDriverPath);
                    m_hService = CreateService(
                        hSCManager,            // SCManager database
                        m_NTDriverName,          // name of service
                        m_NTDriverName,          // name to display
                    ...
                    if(m_hService == NULL)
                    {
                        LOG(1,L"(NT driver) CreateService #2 failed. 0x%X", GetLastError());
                        bError = TRUE;
                    }
                    else
                        LOG(2,L"InstallNTDriver: 2nd attempt to create service succeeded.");
                }
            }
            else
            {
                LOG(1,L"(NT driver) CreateService #1 failed. 0x%X", GetLastError());
                bError = TRUE;
            }
        }
    }
    ...
    if(bError)
    {
        LOG(1, L"(NT driver) Failed to install driver.");
        HwDrv_UnloadDriver();
    ...
    {
        LOG(1, L"(NT driver) Install complete.");
        return HWDRV_LOAD_SUCCESS;
    }
}
...
static BOOL AdjustAccessRights( void )
{
    ...
    if(!QueryServiceObjectSecurity(m_hService, DACL_SECURITY_INFORMATION, psd, 0, &dwSize))
    {
        ...
        {
            LOG(1,L"QueryServiceObjectSecurity #1 failed. 0x%X", GetLastError());
            bError = TRUE;
        }                
    }
    ...
    if(!bError)
    {
        ...
        {
            LOG(1,L"QueryServiceObjectSecurity #2 failed. 0x%X",GetLastError());
            bError = TRUE;                       
        }
    }
    ...
    if(!bError)
    {
        ...
        {
            LOG(1,L"GetSecurityDescriptorDacl failed. 0x%X",GetLastError());
            bError = TRUE;
        }
    }
    ...
    if(!bError)
    {
        ...
        if (!AllocateAndInitializeSid(&SIDAuthWorld, 1,
                     ...
                     &pSIDEveryone))
        {
            LOG(1,L"AllocateAndInitializeSid() failed. 0x%X",GetLastError());
            bError = TRUE;
        }
        else
        {
            ...
            ea.Trustee.ptstrName = reinterpret_cast<WCHAR *>(pSIDEveryone);
    
            dwError = SetEntriesInAcl(1, &ea, pacl, &pNewAcl);
            if(dwError != ERROR_SUCCESS)
            {
                LOG(0,L"SetEntriesInAcl failed. %d", dwError);
                bError = TRUE;
            }
        }
    ...
    if(!bError)
    {
        ...
        {
            LOG(1,L"InitializeSecurityDescriptor failed. 0x%X",GetLastError());
            bError = TRUE;
        }
    }
    ...
    if(!bError)
    {
        ...
        {
            LOG(1, L"SetSecurityDescriptorDacl", GetLastError());
            bError = TRUE;
        }
    }
    ...
    if(!bError)
    {
        ...
        {
            LOG(1, L"SetServiceObjectSecurity", GetLastError());
            bError = TRUE;
        }
    }
...
BOOL HwDrv_UnInstallNTDriver( void )
{
    ...
    LOG(1, L"Attempting to uninstall NT driver.");

    if (m_bWindows95)
    {
        LOG(1,L"(NT driver) Uninstall not needed with win9x/ME.");
    }
    else
    {
        HwDrv_UnloadDriver();
        ...
        if(!bError)
        {
            ...
            if(m_hService == NULL)
            {
                ...
                {
                    LOG(1,L"(NT driver) Service does not exist, no need to uninstall.");
                    CloseServiceHandle(m_hService);
                    m_hService = NULL;
                    return TRUE;
                }
                ...
                {
                    LOG(1,L"(NT driver) Unable to open service. 0x%X",GetLastError());
                    bError = TRUE;
                }
            }
        }
        ...
        if(!bError)
        {
            if (DeleteService(m_hService) == FALSE)
            {
                LOG(1,L"DeleteService failed, error 0x%X", GetLastError());
                bError = TRUE;
            }
        }
        ...
        {
            HwDrv_UnloadDriver();
            LOG(1, L"Failed to uninstall driver.");
            return FALSE;
        }
        LOG(1, L"Uninstall NT driver complete.");
    }
    return TRUE;
}
                         ...
                         )
{
    ...
    else
    {
        ...
        {
            LOG(2, L"DeviceIoControl returned an error = 0x%X For Command GetPCIInfo. This is probably by design (PCI bus scan), do not worry.", GetLastError());
        }
        else
        {
            LOG(1, L"DeviceIoControl returned an error = 0x%X For Command 0x%X", GetLastError(), dwIOCommand);
        }
        return GetLastError();
    }
}
